"use client";
import { motion } from "framer-motion"; // Correction de l'import si nécessaire
...
const ProjectCard = ({ title, description, tags, index }: ProjectProps) => {
  return (
    <motion.div 
      // L'effet de reveal : glissement léger et opacité
      initial={{ opacity: 0, x: 50 }} 
      whileInView={{ opacity: 1, x: 0 }}
      viewport={{ once: false, amount: 0.2 }}
      transition={{ 
        duration: 0.8, 
        ease: [0.215, 0.61, 0.355, 1], // Ease out cubic pour plus de naturel
        delay: index * 0.1 
      }}
      className="bg-[#F3EEE7] rounded-[24px] p-6 md:p-8 border border-[#2C2420]/5 
                 hover:shadow-xl transition-all duration-500 group h-full flex flex-col justify-between"
    >
      <div className="space-y-6">
        {/* Ratio image plus élégant (4:3) */}
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            {tags.map((tag, i) => (
              <span key={i} className="text-[9px] px-3 py-1 border border-[#6F4E37]/20 
          ...
          <h3 className="text-2xl md:text-3xl font-bold text-[#2C2420] tracking-tight italic">
            {title}
          </h3>
          <p className="text-[#6F4E37]/80 text-sm leading-relaxed font-light line-clamp-2">
            {description}
          </p>
        </div>
      </div>
